...
>
> _Disabled local rolling-Absorb runtime foundation (2026-07-23, local-only):_
> scripts/radar/rolling-microstructure-producer.mjs is a one-shot, disabled-by-default
> local wrapper around the pure rolling core. With no WORKER_RADAR_ROLLING_ENABLED=true
> it performs zero network calls and zero POSTs; enabled execution is dry-run unless the
> separate WORKER_RADAR_ROLLING_POST_ENABLED=true is set. POST mode refuses to begin
> without BOT_WORKER_TOKEN and CONTROL_BASE_URL. It calls only public Binance futures
> aggTrades and depth; tests inject every fetch. Payload rows carry measurement
> time/window/sample/source metadata; thin, malformed, stale, or incomplete fields are
> omitted, and the snapshot normalizer rejects malformed, thin, or stale foundation rows
> before merge. The producer stamps trusted:false, so it cannot make strict Absorb,
> ENTRY_READY, or Telegram live. No scheduler, workflow, production activation,
> private/signed Binance endpoint, trading, or Telegram path was added.
> _RADAR Focus checklist layout hotfix (2026-07-23, local-only):_ The emoji-free checklist had one right-aligned flex value column, which could split `DATA OFF` and `ADVISORY` vertically. It now uses stable left-aligned grid columns **Gate / Status / Meaning**, with non-wrapping status badges; all wording, gate values, and advisory boundaries remain unchanged. Technical details remain collapsed by default and the STALE explanation remains inside it. Cache-bust is **`?v=6i5` -> `?v=6i6`**. No backend, strict-Absorb runtime, scoring, ENTRY_READY, Telegram, producer, endpoint, or trading change.
>
