#!/usr/bin/env node
/** Local, one-shot public-futures rolling measurement producer. Disabled by default. */
...
async function collectSymbol({ fetchImpl, waitFn, baseUrl, symbol, now, windowMs, minSamples, depthIntervalMs }) {
  ...
  // Malformed trades reject the sample; a boundary trade outside the requested
  // window does not (this feed already queries by startTime/endTime, so the
  // out-of-window count should be ~0 — it just must not be treated as corruption).
  if (tradeValidation.malformed || tradeValidation.trades.length < minSamples || !first || !second || !Array.isArray(klines) || klines.length < 30) return null;
